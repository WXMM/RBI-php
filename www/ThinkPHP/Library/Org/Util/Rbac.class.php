...
class Rbac {
    ...
    static public function getControllerAccess($authId){
//        自定义类获取控制器下的权限
        // Db方式权限数据
        $db     =   Db::getInstance(C('RBAC_DB_DSN'));
        $table = array('role'=>C('RBAC_ROLE_TABLE'),'user'=>C('RBAC_USER_TABLE'),'access'=>C('RBAC_ACCESS_TABLE'),'node'=>C('RBAC_NODE_TABLE'));
...
//            }
            ...
            foreach($modules as $key=>$module) {
                $moduleId	 =	 $module['id'];
                $moduleName = $module['name'];
                $sql    =   "select node.id,node.name,node.status,node.title from ".
                    $table['role']." as role,".
                    $table['user']." as user,".
                    $table['access']." as access ,".
                    $table['node']." as node ".
                ...
                $action = array();
                foreach ($rs as $a){
                    $action[$a['name']]["id"]	 =	 $a['status'];
                    $action[$a['name']]["name"]	 =	 $a['title'];
                }
                // 和公共模块的操作权限合并
//                $action += $publicAction;
                $access[strtoupper($appName)][strtoupper($moduleName)]   =  array_change_key_case($action,CASE_UPPER);
    ...
    static public function getUserRole($authId){
//        自定义类获取控制器下的权限
        // Db方式权限数据
        $db     =   Db::getInstance(C('RBAC_DB_DSN'));
        $table = array('role'=>C('RBAC_ROLE_TABLE'),'user'=>C('RBAC_USER_TABLE'),'access'=>C('RBAC_ACCESS_TABLE'),'node'=>C('RBAC_NODE_TABLE'));
        $sql    =   "select role_id from ".$table['user']." as user where user.user_id='{$authId}'";
        $apps =   $db->query($sql);
        $roleId=$apps[0]['role_id'];
        $role   =   "select * from  ".'tb_role'."  as role where role.id = '{$roleId}' ";
        $role =   $db->query($role);
        return $role[0];
    }
}