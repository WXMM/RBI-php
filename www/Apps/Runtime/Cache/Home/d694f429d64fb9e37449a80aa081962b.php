...
<div style="width: 98%;margin-right: auto;margin-left: auto">
    ...
    <table class="bordered searches">
        <thead>
        <tr><th>公司</th><th>罐区/车间</th><th>位号</th><th>储罐名称</th><th>上次维修时间</th><th>操作</th></tr>
        </thead>
        <tbody>
        <?php if(is_array($plant)): $i = 0; $__LIST__ = $plant;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$fou): $mod = ($i % 2 );++$i;?><tr  class="brief" data-row-id="<?php echo ($fou["id"]); ?>"><td style="display: none" class="id"><?php echo ($fou["id"]); ?></td>
                ...
                <td><?php if(empty($fou["Plantmaintance"]["Last_plantMaintance"])): ?>-<?php endif; ?>
                    <?php echo ($fou["Plantmaintance"]["Last_plantMaintance"]); ?></td>
                <td class="manage">
                    <a class="detail Access" data-access="<?php echo ($Access['DETAIL']['id']); ?>" href="MaintenanceRecordlist.html?id=<?php echo ($fou["id"]); ?>"><span>详细</span></a>
                </td>
