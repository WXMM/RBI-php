...
<div class="tankInfoDetail" style="width:98%;margin-right: auto;margin-left: auto">
    <table class="borderedDetail">
        <tbody>
        <tr>
            <td><p>所在厂区:</p></td><td><?php echo ($plant[0]["factoryId"]); ?></td>
            <td><p>所在车间:</p></td><td><?php echo ($plant[0]["workshopId"]); ?></td>
            <td><p>设备名称:</p></td><td><?php echo ($plant[0]["plantName"]); ?></td></tr>
        <tr><td><p>设备位号:</p></td><td><?php echo ($plant[0]["plantNO"]); ?></td>
            <td><p>检验单位:</p></td><td><?php echo ($PlantInspectRecord[0]["testCompany"]); ?></td>
            <td><p>检验类型:</p></td><td><?php echo ($PlantInspectRecord[0]["testType"]); ?></td></tr>
        <tr><td><p>检验日期:</p></td><td><?php echo ($PlantInspectRecord[0]["testDate"]); ?></td>
            <td><p>下次检验日期:</p></td><td><?php echo ($PlantInspectRecord[0]["nextTestDate"]); ?></td>
            <td><p></p></td><td></td></tr>
        <tr><td><p>备注:</p></td><td colspan="5"><?php echo ($PlantInspectRecord[0]["testRemark"]); ?></td>
            </tr>
        </tbody>
    </table>
<div class="tabbable">
    ...
    <div id="content">
        ...
        <a data-messageId="<?php echo ($PlantInspectRecord[0]["id"]); ?>" data-verifierLevel="<?php echo ($PlantInspectRecord[0]["verifierLevel"]); ?>" class="btn btn-primary Access" id="verifiedNo" data-access="<?php echo ($Access['VERIFIED']['id']); ?>" style=" background-color: #0b80c9;height:30px;float: right;padding: 5px;border: 0px;margin-bottom: 10px;margin-left: 10px"><span class="glyphicon glyphicon-remove" style="margin-right:5px "></span>审核不通过</a>
        <a data-messageId="<?php echo ($PlantInspectRecord[0]["id"]); ?>" data-verifierLevel="<?php echo ($PlantInspectRecord[0]["verifierLevel"]); ?>" class="btn btn-primary Access" id="verifiedYes" data-access="<?php echo ($Access['VERIFIED']['id']); ?>" style=" background-color: #0b80c9;height:30px;float: right;padding: 5px;border: 0px;margin-bottom: 10px;margin-left: 10px"><span class="glyphicon glyphicon-ok" style="margin-right:5px "></span>审核通过</a>

        <div id="tab1" class="tab">
            <div style="width: 100%;margin:10px auto;">
                <div class="choice" style="margin-bottom: 20px">
                    <span> 检验部位：</span>
                    <input id="bottomBorderBox" type="checkbox" name="bottomBorderBox" value="<?php echo ($PlantInspectRecord[0]["isBottomTest"]); ?>"  disabled="disabled"/><span>底板</span>
                    <input id="wallBorderBox" type="checkbox"  name='wallBorderBox' value="<?php echo ($PlantInspectRecord[0]["isWallTest"]); ?>"  disabled="disabled"/><span>壁板</span>
                    <input id="topBorderBox" type="checkbox" name="topBorderBox" value="<?php echo ($PlantInspectRecord[0]["isTopTest"]); ?>" disabled="disabled" /><span>顶板</span>
                </div>
                <div style="width: 100%;margin:10px auto;">
                    <!-- box容器 start -->
                    <div class="box-container bottomBorderBox" style="display: none">
                        ...
                        <div class="detail">
                            <ol>
                                <table class="borderedDetail">
                                    <tbody>
                                    <tr><td><p>底板检验日期:</p></td><td><?php echo ($PlantInspectRecord[0]["testDate_bottom"]); ?></td>
                                        <td><p>底板检验类型:</p></td><td><?php echo ($PlantInspectRecord[0]["testType_bottom"]); ?></td></tr>
                                    <tr><td><p>储罐基础沉降:</p></td><td><?php echo ($PlantInspectRecord[0]["testBasicSettle_bottom"]); ?></td>
                                        <td><p>底板检验有效性:</p></td><td><?php echo ($PlantInspectRecord[0]["testMethod_bottom"]); ?></td></tr>
                                    <tr>
                                        <td><p>中幅板最小测量厚度(mm):</p></td><td><?php echo ($PlantInspectRecord[0]["testMinThickness1_bottom"]); ?></td>
                                        <td><p>边缘板最小测量厚度(mm):</p></td><td><?php echo ($PlantInspectRecord[0]["testMinThickness2_bottom"]); ?></td>
                                    </tr>
                                    </tbody>
                                </table>
                            </ol>
                    ...
                    <div class="detail">
                        <ol>
                            <table class="bordered">
                                <thead><th>壁板层号</th><th>检验日期</th><th>检验有效性</th><th>最小测量厚度</th></thead>
                                <tbody>
                                <?php if(is_array($PlantInspectRecordWall)): $i = 0; $__LIST__ = $PlantInspectRecordWall;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$rD): $mod = ($i % 2 );++$i;?><tr><td><?php echo ($rD["layerNO"]); ?></td><td><?php echo ($rD["testDate"]); ?>
                                    </td><td><?php echo ($rD["testMethod"]); ?></td><td><?php echo ($rD["minThickness"]); ?></td></tr><?php endforeach; endif; else: echo "" ;endif; ?>
                                </tbody>
                            </table>
                        </ol>
                    </div>
                    ...
                    <div class="box-container topBorderBox" style="display: none">
                        ...
                        <div class="detail">
                            <ol>
                                <table class="borderedDetail">
                                    <tbody>
                                    <tr><td><p>顶板检验日期:</p></td><td><?php echo ($PlantInspectRecord[0]["testDate_top"]); ?></td>
                                        <td><p>顶板检验有效性:</p></td><td><?php echo ($PlantInspectRecord[0]["testMethod_top"]); ?></td></tr>
                                    <tr><td><p>顶板最小测量厚度(mm):</p></td><td><?php echo ($PlantInspectRecord[0]["testMinThickness_top"]); ?></td>
                                        <td><p></p></td><td></td></tr>
                                    </tbody>
                                </table>
                            </ol>
        ...
        <div id="tab2" class="tab">
            <table class="bordered">
                ...
                <tbody>
                <?php if(is_array($checkResult)): $i = 0; $__LIST__ = $checkResult;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$rD): $mod = ($i % 2 );++$i;?><tr data-alarm="<?php echo ($rD["checkResult"]); ?>">
                        <td><?php echo ($rD["checkItem"]); ?></td>
                        <td>
                            <?php switch($rD["checkResult"]): case "1": ?>√<?php break;?>
                                <?php case "0": ?>×<?php break;?>
                                <?php case "2": ?>\<?php break; endswitch;?>
                    </td>
                        <td><?php echo ($rD["remark"]); ?></td></tr><?php endforeach; endif; else: echo "" ;endif; ?>
                </tbody>
            </table>
        </div>
        <div id="tab3" class="tab" style="margin-bottom: 20px">
            <table class="bordered">
                <thead>
        ...
        $("#verifiedNo").click(function(){
            ...
            if(confirm("是否确认审核不通过？")){
                $.post("<?php echo U('Public/verified');?>",
                        {tableName:"tb_planttestrecord",id:id,verifierLevel:verifierLevel,verifyResult:2},function(data){
                            alert(data.msg);
